...
class ban extends Command {
  ...
  async handle ({ args, client, msg }, responder) {
    ...
    else if (!msg.mentions[0]) return responder.send(`${lang.rdeny} ${lang.bannotvalid}`)
    else if (user.id === msg.author.id) return responder.send(`${lang.rquestion} ${lang.banself}`)
    else if (user.id === client.user.id) return responder.send(`${lang.rdeny} ${lang.banbot}`)
    else {
      try {
        ...
        if (msg.guild.channels.get(db.get(`banlog_${msg.channel.guild.id}`)) != null) {
          client.createMessage(msg.guild.channels.get(db.get(`banlog_${msg.channel.guild.id}`)), { embed: {
            color: color,
            title: 'Member Ban',
